...
async function tsToBlockNum(network, ts) {
  try {
    const explorer_base_url = network === '1' ? 'https://api.etherscan.io' : networks[network].explorer // TODO(zzuziak): figure out workaround for this
    ...
    return responseData.result?.blockNumber ?? responseData.result; // TODO(zzuziak): run script for all networks to find exceptions
  } catch (error) {
    console.log("Explorer failed"); // TODO(zzuziak): remove me
    console.log(error); // TODO(zzuziak): remove me unless logging needed

    const provider = snapshot.utils.getProvider(network); // TODO(zzuziak): refactor into calculateBlockNumber
